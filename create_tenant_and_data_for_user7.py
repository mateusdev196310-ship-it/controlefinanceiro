...
from financas.models import Categoria, Conta, Banco, Tenant

User = get_user_model()

print("=== Criando tenant e dados de teste para usuário testuser (ID: 7) ===")
...
print(f"\nCriando tenant para ID {tenant_id}...")
tenant, created = Tenant.objects.get_or_create(
    id=tenant_id,
    defaults={
        'nome': f'Tenant for {user.username}',
        'codigo': f'USER_{user.username.upper()}',
        'ativo': True
    }
)
if created:
    ...
    print(f"  - Tenant já existe: {tenant.nome} (ID: {tenant.id})")

# Associar usuário ao tenant
if user not in tenant.usuarios.all():
    tenant.usuarios.add(user)
    print(f"  ✓ Usuário {user.username} associado ao tenant")
else:
    print(f"  - Usuário {user.username} já associado ao tenant")
