...
def fix_schemas_with_views():
    print('=== CORRIGINDO SCHEMAS INDIVIDUAIS (REMOVENDO VIEWS PRIMEIRO) ===')
    ...
    for schema_tuple in schemas:
        ...
        print(f'\nCorrigindo schema: {schema_name}')
        
        try:
            # Definir search_path para o schema individual
            cursor.execute(f'SET search_path TO {schema_name};')
            
            # 1. Remover views que podem estar causando dependências
            print('  Removendo views...')
            
            # Listar todas as views no schema
            cursor.execute("""
            ...
            for view_tuple in views:
                ...
                try:
                    ...
                    print(f'    - Erro ao remover view {view_name}: {e}')
            
            # 2. Corrigir tabela financas_conta
            ...
            if 'saldo' not in existing_columns and ('saldo_inicial' in existing_columns or 'saldo_atual' in existing_columns):
                ...
                print('    - Coluna saldo adicionada e valores copiados')
            
            # Agora remover colunas antigas com CASCADE
            if 'saldo_inicial' in existing_columns:
                cursor.execute('ALTER TABLE financas_conta DROP COLUMN IF EXISTS saldo_inicial CASCADE;')
                print('    - Coluna saldo_inicial removida')
            if 'saldo_atual' in existing_columns:
                cursor.execute('ALTER TABLE financas_conta DROP COLUMN IF EXISTS saldo_atual CASCADE;')
                print('    - Coluna saldo_atual removida')
            ...
            # 3. Corrigir tabela financas_transacao
